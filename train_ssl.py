...
def get_args_parser():
    ...
    parser.add_argument("--lambda_weight", default=0.1, type=float, help="weighing factor")
...
def train(args):
    ...
    sampler = torch.utils.data.DistributedSampler(dataset_val, shuffle=True)
    ...
    student = utils.MultiCropWrapper(student, MLPHead(args.mlp_head_in, args.out_dim, args.use_bn_in_head) , args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, view_pred_loss,
            ...
            epoch, fp16_scaler, args , criterion)
        test_stats = validate_network(val_loader, student , criterion)
...
def train_one_epoch(student, teacher, teacher_without_ddp, view_pred_loss, data_loader,
                    ...
                    fp16_scaler, args , criterion):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images,target) in enumerate(metric_logger.log_every(data_loader, 10, header)):   #add mask here
...
@torch.no_grad()
def validate_network(val_loader, model, criterion):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
