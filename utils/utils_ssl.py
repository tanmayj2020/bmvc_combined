...
class MultiCropWrapper(nn.Module):
    ...
    """
    def __init__(self, backbone, mlphead , args):
    ...
    def forward(self, x ,type="student" , mode="train" , target = None , criterion = None , args=None):
        ...
        if mode == "eval":
            ...
            output_classification = self.linear_classifier(output)
        ...
        if type=="student":
            ...
            if args.cm and not args.mu:
                r = np.random.rand(1)
                if r < args.mix_prob:
                    slicing_idx, y_a, y_b, lam, sliced = cutmix_data(inp, target, args)
                    inp[:, :, slicing_idx[0]:slicing_idx[2], slicing_idx[1]:slicing_idx[3]] = sliced
                    output = self.backbone(inp)
                    output = self.linear_classifier(output)
                    loss_classification =  mixup_criterion(criterion, output, y_a, y_b, lam)
                else:
                    output = self.backbone(inp)
                    output = self.linear_classifier(output)
            ...
            # Mixup only
                ...
                r = np.random.rand(1)
                if r < args.mix_prob:
                    inp, y_a, y_b, lam = mixup_data(inp, target, args)
                    output = self.backbone(inp)
                    output = self.linear_classifier(output)
                    loss_classification =  mixup_criterion(criterion, output, y_a, y_b, lam)
        ...
        # No Mix
            ...
            for end_idx in idx_crops:
                ...
                start_idx = end_idx
            # Run the head forward on the concatenated features.
            return self.mlphead(output) , loss_classification
